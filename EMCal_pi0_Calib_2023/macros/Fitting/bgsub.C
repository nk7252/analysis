...
void fit_2d_histogram(Double_t scale_factor, const std::vector<float>& limits, bool fitEtaPeak = false, int startBin = 1, int endBin = -1)
{
    ROOT::Math::MinimizerOptions::SetDefaultStrategy(2);
    SetsPhenixStyle();
    ...
    for (int i = startBin; i <= endBin; ++i)
    {
        ...
        if (fitEtaPeak)
        {
            // Set initial guesses for the second Gaussian (eta peak)
            combinedFit->SetParameter(8, gausFit->GetParameter(0) / 3);  // Assume a smaller amplitude
            combinedFit->SetParameter(9, (limits[4] + limits[5]) / 2.0); // Center of the eta peak range
            combinedFit->SetParameter(10, (limits[5] - limits[4]) / 4.0); // Initial sigma guess based on range width
        }
        ...
        TF1 *doubleGaussFit = new TF1("doubleGaussFit", doubleGauss, limits[2], limits[3], 10);
        ...
        if (fitEtaPeak)
        {
            ...
            doubleGaussFit->SetParameter(8, limits[4]);
            doubleGaussFit->SetParameter(9, limits[5]);
        }
