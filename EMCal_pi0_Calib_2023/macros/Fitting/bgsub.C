...
void fit_2d_histogram(Double_t scale_factor, float leftmost_gauslimit, float rightmost_gauslimit, bool fitEtaPeak = false, int startBin = 1, int endBin = -1)
{
    ...
    for (int i = startBin; i <= endBin; ++i)
    {
        ...
        fitInfo->AddText(Form("Mean = %f +/- %f", gausFit2->GetParameter(1), gausFit2->GetParError(1)));
        fitInfo->AddText(Form("Sigma = %f +/- %f", gausFit2->GetParameter(2), gausFit2->GetParError(2)));
        fitInfo->AddText(Form("Relative Width: %f", gausFit2->GetParameter(2) * 100.0f / gausFit2->GetParameter(1)));
        fitInfo->AddText(Form("Chi2/NDF = %f / %d = %f", gausFit2->GetChisquare(), gausFit2->GetNDF(), gausFit2->GetChisquare() / gausFit2->GetNDF()));
        fitInfo->Draw();
